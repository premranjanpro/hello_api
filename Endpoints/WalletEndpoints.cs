...
public static class WalletEndpoints
{
    public static void MapWalletEndpoints(this WebApplication app)
    {
        app.MapGet("/api/wallet", async (ClaimsPrincipal cp, IDbConnection db) =>
        {
            ...
            var wallet = await db.QueryFirstOrDefaultAsync<dynamic>("SELECT w.id, w.user_id, 'INR' AS currency, w.created_at, w.updated_at, w.balance FROM wallet_account w WHERE w.user_id=@uid", new { uid });
            if (wallet == null)
            {
                ...
                wallet = await db.QueryFirstOrDefaultAsync<dynamic>("SELECT w.id, w.user_id, 'INR' AS currency, w.created_at, w.updated_at, w.balance FROM wallet_account w WHERE w.user_id=@uid", new { uid });
            }
